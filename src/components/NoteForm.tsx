      ...
      <div className="bg-surface-container rounded-2xl p-4 md:p-6 shadow-sm border border-outline-variant/15 backdrop-blur-sm">
        <form className="space-y-4" onSubmit={handleSubmit} noValidate>
          <div>
            ...
            />
            {errors.title && (
              <p id="title-error" className="text-error text-sm px-4 pt-1" role="alert">
                {errors.title}
              </p>
            )}
          </div>
          <div className="relative">
            <textarea
              className={`w-full bg-surface-container-lowest text-on-surface placeholder-on-surface-variant border-none rounded-xl focus:ring-1 focus:ring-primary/50 px-4 py-4 resize-none outline-none text-body-lg transition-all ${
                errors.content ? 'ring-1 ring-error/50' : ''
              }`}
            ...
            />
            {errors.content && (
              <p id="content-error" className="text-error text-sm px-4 pt-1" role="alert">
                {errors.content}
              </p>
            )}
          </div>
          <div className="flex justify-end pt-2">
            <button
              className="bg-primary-container text-on-primary-container hover:bg-primary-container/90 active:scale-[0.98] transition-all duration-200 px-8 py-3 rounded-xl font-semibold flex items-center gap-2 shadow-sm shadow-primary/10 disabled:opacity-50 disabled:cursor-not-allowed"
              type="submit"
