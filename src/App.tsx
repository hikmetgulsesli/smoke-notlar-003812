...
export default function App() {
  ...
  const renderNotesList = (noteList: typeof notes, emptyMessage?: string) => {
        ...
        return (
          <section className="flex flex-col items-center text-center space-y-6 p-8 rounded-3xl" style={{
            ...
            <div className="space-y-2">
              <h2 className="text-2xl md:text-3xl font-headline font-bold tracking-tight" style={{ color: 'var(--color-on-surface)' }}>{emptyMessage}</h2>
            </div>
          </section>
        );
      }
      ...
      case 'archive':
        return (
          <>
            ...
            {renderNotesList(archiveNotes, 'Henüz arşivlenmiş not yok.')}
          </>
        );
      case 'profile':
        return (
              ...
              <div className="rounded-2xl p-6 border" style={{
                ...
                <div className="w-full h-4 rounded-full overflow-hidden" style={{ backgroundColor: 'var(--color-surface-container-high)' }}>
                  <div
                    ...
                    style={{
                      width: `${notes.length > 0 ? (completedCount / notes.length) * 100 : 0}%`,
                      backgroundColor: 'var(--color-primary-container)',
                    }}
                  />
                </div>
                <p className="text-sm mt-3" style={{ color: 'var(--color-on-surface-variant)' }}>
                  {notes.length > 0 ? `${Math.round((completedCount / notes.length) * 100)}% tamamlandı` : 'Henüz not yok.'}
                </p>
              </div>
            )}
          </section>
